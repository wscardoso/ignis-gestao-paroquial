...
interface MarriageRegistryProps {
    tenantId: string;
}

export const MarriageRegistry: React.FC<MarriageRegistryProps> = ({ tenantId }) => {
    ...
    const fetchRecords = async () => {
        ...
        try {
            ...
            setRecords(data.filter((r: Sacrament) => r.type === 'marriage'));
        } catch (error) {
    ...
    return (
        <div className="module-container">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <h2 style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.5rem' }}>
                            <Scroll className="text-accent" /> Sacramenta: Matrimônios
                        </h2>
                        <p style={{ opacity: 0.7, fontSize: '0.9rem' }}>Gestão de processos e registros matrimoniais.</p>
                    </div>
                    {view === 'list' ? (
                ...
                {view === 'list' && (
                    <form onSubmit={handleSearch} className="search-bar" style={{ marginTop: '20px', display: 'flex', gap: '10px' }}>
                        <div className="search-input-wrapper" style={{ flex: 1, position: 'relative' }}>
                            ...
                            <input
                                type="text"
                                placeholder="Buscar por noivo ou noiva..."
                                value={searchQuery}
            ...
            {view === 'list' ? (
                <div className="records-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
                    {isLoading ? (
                        ...
                        records.map(record => (
                            <div key={record.id} className="record-card glass hover-effect" style={{ padding: '20px', position: 'relative' }}>
                                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '15px' }}>
                                    <div className="avatar" style={{ width: '40px', height: '40px', backgroundColor: 'rgba(236, 72, 153, 0.1)' }}>
                                ...
                                <div style={{ marginTop: '15px', paddingTop: '15px', borderTop: '1px solid rgba(255,255,255,0.05)', fontSize: '0.85rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        <span style={{ opacity: 0.5 }}>Contraparte</span>
                                        <span>{record.details?.spouse || 'Não informado'}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                                        <span style={{ opacity: 0.5 }}>Testemunha</span>
                                        <span>{record.details?.witness1 || 'Não informado'}</span>
                                    </div>
                                </div>
                        ...
                        <div className="empty-state glass" style={{ gridColumn: '1/-1', padding: '40px', textAlign: 'center', opacity: 0.5 }}>
                            ...
                            <p>Nenhum registro de matrimônio encontrado.</p>
                        </div>
                    )}
                </div>
            ) : (
